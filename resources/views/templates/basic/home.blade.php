                                ...
                                <div class="row mb-4">
                                    <div class="col-lg-6 col-12 col-md-6 mt-75">
                                        <div>
                                            ...
                                            <span class="text-hm" style="width: 100%"><span class="text-black"> The Ultimate
                                                    Destination for</span>
                                                    <span class="text-yel"> Freelance Talent</span> <span
                                                    class="text-black">and</span><span class="text-yel"> Quality
                                                    Services!</span></span>
                                            <div class="img-mobile mt-n3">
                                                <img src="{{ asset($activeTemplateTrue . 'frontend/images/Group1.png') }}"
                                                    alt="" class="">
                                            </div>


                                            <span class="text-p text-mobile">Join us to get things done!</span>
...
@push('script')
    ...
    <script>
        $(document).ready(function() {
            let textentre = document.getElementById("search-footer")
            let selected = $('#select-type').val()
            textentre.addEventListener("keydown", (e) => {
                if (e.key == 'Enter') {
                    let input = textentre.value
                    if (selected == 'service') {
                        var url = "{{ route('service.search', ':id') }}";
                        url = url.replace(':id', 'search=' + input.split(' ').join('+'));
                        location.href = url
                    }
                    if (selected == 'product') {
